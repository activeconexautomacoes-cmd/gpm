...
import { usePayables, useFinancialCategories, useBankAccounts, useCostCenters, useSuppliers } from "@/hooks/useFinancialModules";
...
interface PayableFormProps {
    initialData?: Database["public"]["Tables"]["financial_payables"]["Row"];
    onSuccess?: () => void;
...
export function PayableForm({ initialData, onSuccess, trigger, open: externalOpen, onOpenChange: setExternalOpen }: PayableFormProps) {
    const [internalOpen, setInternalOpen] = useState(false);
    const open = externalOpen !== undefined ? externalOpen : internalOpen;
    const setOpen = setExternalOpen !== undefined ? setExternalOpen : setInternalOpen;
    const { currentWorkspace } = useWorkspace();
    const { createPayable, updatePayable } = usePayables(currentWorkspace?.id);
    const { suppliers, createSupplier } = useSuppliers(currentWorkspace?.id);
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        // Validation check when allocation is disabled
        if (!enableAllocation && !values.category_id) {
            form.setError("category_id", { type: "manual", message: "Categoria é obrigatória" });
        ...
        try {
            ...
            // Resolve supplier
            let finalSupplierId = initialData?.supplier_id || null;
            if (values.supplier_name && values.supplier_name !== initialData?.supplier_name) {
                const existing = suppliers?.find(s => s.name.toLowerCase() === values.supplier_name?.toLowerCase());
                if (existing) {
                    finalSupplierId = existing.id;
                } else if (values.supplier_name.trim() !== "") {
            ...
            if (initialData?.id) {
                ...
                toast({ title: "Despesa atualizada", description: "O lançamento foi atualizado com sucesso." });
            } else {
                // CREATE logic
                let category_id = values.category_id;
                let cost_center_id = values.cost_center_id || null;

                if (enableAllocation && !values.repeat) { // Allocation only for single launches for now
                    const totalAllocated = allocations.reduce((sum, item) => sum + item.amount, 0);
                    if (Math.abs(totalAllocated - values.amount) > 0.05) {
                        toast({ title: "Erro no rateio", description: "O valor total do rateio deve ser igual ao valor do lançamento.", variant: "destructive" });
                        return;
                ...
                if (values.repeat && values.repeat_type === 'installments' && values.installments && values.installments.length > 1) {
                    ...
                    const promises = values.installments.map((inst) => {
                        return createPayable.mutateAsync({
                            item: {
                                workspace_id: workspaceId,
                    ...
                    await Promise.all(promises);
                    toast({ title: "Despesas recorrentes lançadas", description: `${values.recurrence_count} lançamentos foram criados com sucesso.` });
                } else {
                    // Single creation
                    const createdItem = await createPayable.mutateAsync({
                        item: {
                            workspace_id: workspaceId,
                            description: values.description,
                            title: values.description,
                    ...
                    if (pendingFiles.length > 0) {
                        for (const file of pendingFiles) {
                            await uploadAttachment.mutateAsync({ file, workspaceId: workspaceId, payableId: createdItem.id });
                        }
                    }

                    toast({ title: "Despesa lançada", description: "O lançamento foi criado com sucesso." });
                }
            }
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-5xl p-0 overflow-hidden border-none shadow-2xl h-[95vh] flex flex-col">
                <div className="bg-slate-50 flex flex-col h-full">
                    <DialogHeader className="p-6 bg-white border-b shrink-0 flex-row items-center justify-between shadow-sm">
                        ...
                        <form
                            onSubmit={form.handleSubmit(onSubmit, (errors) => {
                                ...
                                toast({
                                    ...
                                    description: "Por favor, preencha a conta bancária e a data de pagamento para os lançamentos marcados como pagos.",
                                    variant: "destructive",
                                });
                            })}
                        ...
                        >
                            <div className="p-8 space-y-8 pb-24">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                        <div className="md:col-span-1">
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        <FormLabel className="text-xs font-bold text-slate-500 uppercase">Fornecedor</FormLabel>
                                                        <FormControl>
                                                            <SupplierCombobox
                                                                value={field.value || ""}
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        ...
                                                        <FormControl>
                                                            <Input
                                                                className="bg-slate-50/50 border-slate-200 focus:ring-red-500/20"
                                        ...
                                        <div>
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        <FormLabel className="text-xs font-bold text-slate-500 uppercase text-red-600">Valor *</FormLabel>
                                                        <div className="relative">
                                                            ...
                                                            <FormControl>
                                                                <Input
                                                                    type="number"
                                                                    step="0.01"
                                                                    className="pl-9 bg-slate-50/50 border-slate-200 focus:ring-red-500/20 text-right font-black text-lg"
                                                                    {...field}
                                                                />
                                                            </FormControl>
                                                        </div>
                                ...
                                <div className="bg-slate-100/50 rounded-xl border border-dashed border-slate-300 p-6 flex items-center justify-between group hover:bg-slate-100 transition-all">
                                    <div className="flex items-center gap-4">
                                        <div className={`h-10 w-10 rounded-full flex items-center justify-center transition-colors ${watchRepeat ? 'bg-red-600 text-white' : 'bg-white text-slate-400 border'}`}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                        ...
                                        <div>
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        <FormLabel className="text-xs font-bold text-slate-500 uppercase">Conta de pagamento</FormLabel>
                                                        <Select onValueChange={field.onChange} value={field.value}>
                                                            <FormControl>
                                                                <SelectTrigger className="bg-slate-50/50 border-slate-200">
                                                                    <SelectValue placeholder="Selecione..." />
                                        ...
                                        <div className="flex items-end pb-2 gap-6">
                                            <FormField
                                                ...
                                                name="is_paid"
                                                render={({ field }) => (
                                                    <div className="flex items-center space-x-2 bg-slate-50 px-4 py-2 rounded-lg border border-slate-200 hover:border-red-300 transition-colors cursor-pointer group">
                                                        <Switch
                                                            id="pago-status"
                                                            checked={field.value}
                                                            ...
                                                            className="data-[state=checked]:bg-red-600"
                                                        />
                                                        <label htmlFor="pago-status" className="text-xs font-bold text-slate-600 cursor-pointer group-hover:text-red-700">PAGO</label>
                                                    </div>
                                                )}
                                            />
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pt-4 border-t border-dashed animate-in fade-in slide-in-from-top-2 duration-300">
                                            <div>
                                                <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem className="flex flex-col">
                                                            <FormLabel className="text-xs font-bold text-slate-500 uppercase">Data do Pagamento *</FormLabel>
                                                            <Popover>
                                                                <PopoverTrigger asChild>
                                                                    <FormControl>
                                                                        <Button
                                ...
                                <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                                    <Tabs defaultValue="obs" className="w-full">
                                        <TabsList className="bg-slate-50 border-b w-full rounded-none justify-start h-12 px-6 gap-6">
                                            <TabsTrigger value="obs" className="data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-red-600 data-[state=active]:shadow-none rounded-none px-0 text-xs font-bold text-slate-500 uppercase tracking-widest h-12">
                                                <FileText className="h-4 w-4 mr-2" /> Observações
                                            </TabsTrigger>
                                            <TabsTrigger value="anexo" className="data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-red-600 data-[state=active]:shadow-none rounded-none px-0 text-xs font-bold text-slate-500 uppercase tracking-widest h-12">
                                                        ...
                                                        <FormItem>
                                                            <FormControl>
                                                                <textarea
                                                                    ...
                                                                    className="w-full min-h-[120px] bg-slate-50/30 border border-slate-200 rounded-xl p-4 text-sm focus:ring-1 focus:ring-red-500/20 outline-none transition-all placeholder:italic"
                                                                    {...field}
                                                                />
                                                            </FormControl>
                                                        </FormItem>
                            ...
                            <div className="p-6 bg-white border-t flex justify-between items-center absolute bottom-0 left-0 right-0 shadow-[0_-10px_30px_rgba(0,0,0,0.03)] rounded-b-3xl">
                                ...
                                <div className="flex gap-3">
                                    <Button
                                        type="submit"
                                        className="bg-red-600 hover:bg-red-700 text-white font-black px-10 rounded-lg shadow-lg shadow-red-100 h-11 group transition-all active:scale-95"
                                    ...
                                    >
                                        ...
                                        {initialData ? "Atualizar lançamento" : "Salvar despesa"}
                                    </Button>
                                    <div className="relative">
                                        <Button
                                            type="submit"
