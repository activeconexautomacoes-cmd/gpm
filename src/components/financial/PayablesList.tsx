...
import { usePayables, useBankAccounts, useFinancialCategories } from "@/hooks/useFinancialModules";
...
import {
    ...
    FileText, Plus, LayoutGrid, X, Filter, Paperclip
} from "lucide-react";
import {
    DropdownMenu,
    DropdownMenuContent,
...
export function PayablesList() {
    ...
    const { payables, isLoading, createPayable, updatePayable, deletePayable } = usePayables(currentWorkspace?.id);
    ...
    // Get related items for installment/recurring deletion
    const relatedPayables = useMemo(() => {
        if (!itemToDelete || !payables) return [];

        // Find items with same reference_code (installments) or is_recurring pattern
        ...
        return payables.filter(p => {
            ...
            if (refCode && p.reference_code === refCode) return true;

            // Fallback for older installments without reference_code
            if (!refCode && baseDescription) {
                const pMatch = p.description.match(/(.+) (\d+)\/(\d+)$/);
                if (pMatch && pMatch[1].trim() === baseDescription && pMatch[3].trim() === outOfMatch) {
                    return true;
                }
            }
    ...
    const handleConfirmDeleteSingle = async (id: string) => {
        setIsDeleting(true);
        try {
            await deletePayable.mutateAsync({ id, workspaceId: currentWorkspace!.id });
    ...
    const handleConfirmDeleteAll = async (ids: string[]) => {
        setIsDeleting(true);
        try {
            await Promise.all(ids.map(id => deletePayable.mutateAsync({ id, workspaceId: currentWorkspace!.id })));
    ...
    }, [payables, search, statusFilter, dateFilter, referenceDate, customRange, bankAccountFilter, categoryFilter, minAmount, maxAmount, quickStatusFilter]);
    ...
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            setSelectedIds(filteredPayables.map(item => item.id));
        } else {
            setSelectedIds([]);
        }
    };
    ...
    const SortIcon = ({ column }: { column: string }) => {
        if (sortConfig.key !== column) return <ChevronsUpDown className="h-3 w-3 ml-1 opacity-50" />;
        return sortConfig.direction === 'asc' ? <ChevronUp className="h-3 w-3 ml-1" /> : <ChevronDown className="h-3 w-3 ml-1" />;
    };

    const periodTotals = useMemo(() => {
        if (!payables) return { paid: 0, paidTotal: 0, pending: 0, pendingTotal: 0, overdue: 0, overdueTotal: 0, today: 0, todayTotal: 0, upcoming: 0, upcomingTotal: 0, total: 0, totalCount: 0 };

        const now = new Date();
        now.setHours(0, 0, 0, 0);

        const baseFiltered = payables.filter(item => {
    ...
    }, [payables, dateFilter, customRange, search, bankAccountFilter, categoryFilter, minAmount, maxAmount]);

    const selectedSum = useMemo(() => {
        if (!payables || selectedIds.length === 0) return 0;
        return payables
    ...
    const executeBatchDelete = async () => {
        const selectedItems = payables?.filter(item => selectedIds.includes(item.id)) || [];
        ...
        if (deletableItems.length === 0) {
            toast({
                ...
                description: "Todos os lançamentos selecionados já estão pagos e não podem ser excluídos.",
                variant: "destructive"
            });
            return;
        }
        ...
        try {
            await Promise.all(deletableItems.map(item => deletePayable.mutateAsync({ id: item.id, workspaceId: currentWorkspace!.id })));

            const message = paidItems.length > 0
                ? `${deletableItems.length} lançamentos excluídos. ${paidItems.length} lançamento(s) pago(s) foram mantidos.`
    ...
    const handleBatchAction = async (action: string, data?: any) => {
        ...
        const confirmMessage = action === 'mark_paid'
                ? `Deseja marcar ${selectedIds.length} lançamentos como pagos?`
                : action === 'mark_pending'
        ...
        try {
            ...
            if (action === 'mark_paid') {
                ...
                await Promise.all(selectedItems.map(item => updatePayable.mutateAsync({
                    item: {
                        id: item.id,
            ...
            toast({
                title: "Sucesso",
                description: `${selectedIds.length} lançamentos atualizados.`,
            });
            setSelectedIds([]);
        } catch (error) {
            toast({
                title: "Erro",
                description: "Ocorreu um erro ao processar a ação em lote.",
                variant: "destructive"
            });
        }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-center gap-2">
                    <PayableForm
                        trigger={
                            <Button className="bg-destructive hover:bg-destructive/90 text-white font-bold flex gap-2">
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                            <DropdownMenuItem>Gasto por Fornecedor</DropdownMenuItem>
                            <DropdownMenuItem>Relação de Pagamentos</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
            ...
            <div className="bg-card border rounded-lg p-5 shadow-sm space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    ...
                    <div className="space-y-2 md:col-span-1">
                        <label className="text-[10px] font-bold uppercase text-muted-foreground">Pesquisar por descrição ou fornecedor</label>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-0 border rounded-lg overflow-hidden bg-white shadow-sm">
                ...
                >
                    <p className="text-[10px] font-bold text-muted-foreground uppercase mb-1">Pagos (R$)</p>
            ...
            {selectedIds.length > 0 && (
                <div className="flex items-center gap-4 bg-primary text-primary-foreground px-4 py-2 rounded-md animate-in slide-in-from-top-2">
                    ...
                    <FinancialBatchActions
                        selectedItems={payables?.filter(item => selectedIds.includes(item.id)) || []}
                ...
                <Table>
                    <TableHeader className="bg-muted/50">
                        <TableRow>
                            ...
                            <TableHead className="text-right font-bold text-xs uppercase">A pagar (R$)</TableHead>
                                ...
                                return (
                                    <TableRow key={item.id} className={`${selectedIds.includes(item.id) ? "bg-primary/5" : "hover:bg-muted/30"} transition-colors group h-14`}>
                                        ...
                                        <TableCell className="py-4">
                                            <div className="flex flex-col gap-1">
                                                <span className="text-[13px] font-semibold text-slate-800 leading-none group-hover:text-primary transition-colors">
                                                    {item.description}
                                                </span>
                                                <div className="flex items-center gap-2 text-[11px]">
                                                    <span className="text-slate-500 font-medium bg-slate-100 px-1.5 py-0.5 rounded">
                                                        {item.financial_categories?.name || 'Sem categoria'}
                                                    </span>
                                                    {item.supplier_name && (
                                        ...
                                        <TableCell>
                                            <Badge variant={getStatusColor(item.status) as 'default'} className="font-bold text-[10px] uppercase border shadow-none flex items-center w-fit gap-1">
                                                {item.status === 'paid' ? 'Pago' :
                                                    item.status === 'partial' ? 'Pagto. Parcial' :
                                            ...
                                            <div className="flex items-center justify-end gap-2 group">
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="w-56">
                                                            ...
                                                            <MarkAsPaid
                                                                ...
                                                                trigger={
                                                                    <DropdownMenuItem onSelect={(e) => e.preventDefault()}>
                                                                        <CheckCircle2 className="h-4 w-4 mr-2 text-success" /> Informar pagamento
                                                                    </DropdownMenuItem>
                                                                }
                                                            />
                                                        )}
                                                        ...
                                                        {item.status === 'paid' && (
                                                            <DropdownMenuItem onClick={() => updatePayable.mutate({ item: { id: item.id, workspace_id: item.workspace_id, status: 'pending' as const, payment_date: null } })}>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center bg-card border rounded-lg p-4 gap-4 shadow-sm">
                ...
                <div className="flex items-center gap-6">
                    <span className="text-sm text-muted-foreground">
                        Mostrando {Math.min((currentPage - 1) * itemsPerPage + 1, sortedPayables.length)} - {Math.min(currentPage * itemsPerPage, sortedPayables.length)} de {sortedPayables.length} registros
                    </span>
                    <div className="flex items-center gap-1">
                        <Button
                            variant="outline"
                                    ...
                                    <>
                                        ...
                                        {paidItems.length > 0 && (
                                            <span className="block text-amber-600 font-medium">
                                                {paidItems.length} lançamento(s) com status "Pago" serão mantidos.
                                            </span>
                                        )}
                ...
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle>Lançamento já pago</DialogTitle>
                                <DialogDescription>
                                    Não é possível excluir um lançamento com status "Pago".
                                </DialogDescription>
                            </div>
                        </div>
                    </DialogHeader>
