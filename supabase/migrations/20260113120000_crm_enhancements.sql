-- Add is_scheduling column to opportunity_stages
ALTER TABLE public.opportunity_stages ADD COLUMN IF NOT EXISTS is_scheduling boolean DEFAULT false;

-- Add is_new_lead column to opportunities
ALTER TABLE public.opportunities ADD COLUMN IF NOT EXISTS is_new_lead boolean DEFAULT true;

-- Update process_quiz_submission definition
...
BEGIN
    ...
    -- Create Opportunity (Default is_new_lead=true from column default)
    ...
    -- LINK BOOKING AND UPDATE OPPORTUNITY AND MOVE STAGE
    IF v_opportunity_id IS NOT NULL AND v_booking_id IS NOT NULL THEN
        ...
        -- 2. Find Scheduling Stage (if any)
        SELECT id INTO v_scheduling_stage_id 
        FROM public.opportunity_stages 
        WHERE workspace_id = v_workspace_id AND is_scheduling = true
        LIMIT 1;
        ...
        SET 
            ...
            current_stage_id = COALESCE(v_scheduling_stage_id, current_stage_id)
